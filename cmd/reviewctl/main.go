...
func main() {
	...
	pf.StringVar(&cfg.Provider, "provider", envDefault("REVIEW_PROVIDER", "claude"), "AI provider: claude, codex, or opencode")
	pf.StringVar(&cfg.Model, "model", envDefault("REVIEW_MODEL", "opus"), "model id passed to the provider CLI (opencode requires provider/model form, e.g. anthropic/claude-sonnet-4-6)")
	pf.StringVar(&cfg.OpencodeBin, "opencode-bin", os.Getenv("REVIEW_OPENCODE_BIN"), "path to opencode binary (defaults to 'opencode' on $PATH)")
