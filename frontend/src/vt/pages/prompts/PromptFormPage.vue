...
const languagePresets: Record<string, { title: string; common?: string; fullPrompts?: boolean; code: string; architecture: string; security: string; tests: string; operability: string }> = {
  ...
  'React Native / Expo': { title: 'Expo Review', code: 'Krzysztof Magiera', architecture: 'Evan Bacon', security: 'Kadi Kraman', tests: 'Maciej Jastrzębski', operability: 'Brent Vatne. Проведи ревью операционной готовности этого MR: EAS Update (OTA) и его безопасность, версионирование и совместимость нативных модулей, crash reporting (Sentry), аналитика, feature flags, graceful degradation при потере сети и на старых устройствах, app store / TestFlight release pipeline, план отката.' },
